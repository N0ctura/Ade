...
export async function startBot(): Promise<void> {
  ...
  client.on("guildMemberAdd", async (member) => {
    try {
      ...
      logger.error({ err, guildId: member.guild.id, userId: member.id }, "Error in guildMemberAdd handler");
    }
  });
  ...
  client.on("guildMemberRemove", async (member) => {
    try {
      ...
      logger.error({ err, guildId: member.guild.id, userId: member.id }, "Error in guildMemberRemove handler");
    }
  });
